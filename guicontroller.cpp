...
void GUIcontroller::runCalculator() {
    ...
    MemoryAdd(APP_Name,"Basic",1,100000);
    ...
    while(true) {
        ...
        if (command.length()>0) {
            if (!command.compare("reset")){
                ...
                process_index += 1;
                MemoryAdd(APP_Name,process_name,process_index,100000);
...
void GUIcontroller::initMemory() {
    ...
    Process OS_1 = mo->Create_Process("OS","Basic Service",1,1048576);
    Process OS_2 = mo->Create_Process("OS","Memory",1,16);

    mo->Add_to_memory(OS_1);
    mo->Add_to_memory(OS_2);
